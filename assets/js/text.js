export const text = {
  texte: {
    Axe_I: {
      titre: `Boite à outils d’intervention pour répondre aux actes d’incivilités et de nuisances dans le cadre de la gestion de la SA HLM`,
      descriptif: `Les atteintes à la tranquillité des espaces collectifs : environnement, propreté et nuisance`,
      sections: {
        _1: {
          titre:
            "Les atteintes à la tranquillité des espaces collectifs : environnement, propreté et nuisance",
          contenu: {
            a: {
              titre:
                "Salissures, abandon d’objets, encombrants et jets de détritus",
              descriptif: `Les jets de détritus et dépôts de matériaux en dehors des espaces réservés et/ou des jours d’enlèvement peuvent provoquer des problèmes d’hygiène, d’odeur, de toxicité, de prolifération d’insectes et de nuisibles.
              La nature des objets déposés (produits toxiques, contondants, etc.) peut induire un risque pour autrui, y compris en compromettant l’intervention des services de secours en cas d’incendie. La responsabilité civile, voire pénale, de l’auteur du dépôt litigieux peut alors être engagée.`,
              qualification: `Le fait de « déposer, d’abandonner, de jeter ou de déverser, en lieu public ou privé, à l’exception des emplacements désignés à cet effet […] des ordures, déchets, déjections, matériaux, liquides insalubres, ou tout autre objet de quelque nature qu’il soit » constitue une contravention de 2ème classe (pouvant aller jusqu’à 150 € d’amende).
              « Est puni de l'amende prévue pour les contraventions de la 5e classe le fait de déposer, d'abandonner, de jeter ou de déverser, en lieu public ou privé, à l'exception des emplacements désignés à cet effet par l'autorité administrative compétente » Article R635-8 du Code Pénal.
              La responsabilité civile peut être mise en cause s’il en résulte un dommage aux tiers (article 1384 du Code Civil).`,
              procedure: `D’abord: Prendre des mesures d’urgence et des dispositions d’enlèvement si nécessaire et si cela provoque un danger immédiat (produit contondants, produits toxiques, etc.)
              En cas de découverte de matières ou d’objets dangereux, ne pas les toucher et prévenir le Manager de Proximité responsable du secteur afin de prévenir la Police pour constater les infractions (présence de produits stupéfiants ou autres dépôts dangereux et illégaux)
              Prévenir la Mairie si les détritus ou les objets sont déposés sur l’espace public.
              Si possible, recueillir des témoignages et/ou identifier les objets déposés pour identifier la/les personnes responsables.
              Ensuite:
              Dans les parties communes ou les espaces extérieurs, si l’auteur est identifié :
              Rappeler le règlement intérieur à la personne identifiée et communiquer sur les coûts engendrés pour la SA HLM par un courrier envoyé par l’agence ;
              Puis:
              Mettre en œuvre des actions de sensibilisation sur les sites particulièrement touchés.
              Si l’auteur des faits est locataire de la SA HLM de l’Oise:
              Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ;
              Envoi d’un courrier de mise en demeure;
              Dresser un procès-verbal;
              Demande de remboursement à l’auteur (hors quittance) par l’agence.`,
            },
            b: {
              titre: "Déjections canines",
              descriptif: `Les déjections canines désignent les excréments produits par les chiens. Elles sont strictement interdites sur les voies publiques, les trottoirs, les espaces verts publics et les aires de jeux pour enfants. Cette interdiction vise à garantir l’hygiène publique.

              Les propriétaires ou détenteurs de chiens ont l’obligation de ramasser immédiatement les déjections canines en utilisant un moyen approprié, quel que soit l’endroit concerné sur le domaine public communal.`,
              qualification: `L'article R. 635-8 prévoit une sanction par une amende de la 2ᵉ classe (68€) pour toute personne déposant, abandonnant, jetant ou déversant, dans un lieu public ou privé, des ordures, déchets, déjections, matériaux, liquides insalubres ou tout autre objet, quelle qu’en soit la nature.`,
              procedure: `D’abord:

              Rappeler les règles sur l’importance de maintenir les lieux propres.

              Ensuite:

              Si le responsable est identifié, mettre l’accroche porte.

              Puis:

              Mettre en œuvre des actions de sensibilisation sur les sites particulièrement touchés (installation de canisettes, des distributeurs de sacs à crotte).
              Si l’auteur des faits est locataire de la SA HLM de l’Oise:

              Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ;
              Envoi d’un courrier de mise en demeure;
              Dresser un procès-verbal;
              Demande de remboursement à l’auteur (hors quittance) par l’agence.`,
            },
            c: {
              titre: "Gestion des véhicules (en infraction?) :",
              cas: [
                {
                  cas: "Véhicule épave",
                  descriptif: `Un véhicule épave est un véhicule dont tout laisse à penser qu’il est abandonné et qui ne peut plus être utilisé pour sa destination d’origine (absence de plaque d’immatriculation, de roues, de portières, de moteur, de tableau de bord, etc.).`,
                  qualification: `Le fait « de déposer, d'abandonner ou de jeter, en un lieu public ou privé, à l'exception des emplacements désignés à cet effet […] une épave de véhicule » constitue une contravention de 5ème classe (pouvant aller jusqu’à 1500 € d’amende) selon l’article R635-8 NCP).

                  L'article L541-1 du Code de l'environnement considère qu'un véhicule hors d'usage (VHU) est un déchet. Si un véhicule est démonté ou éliminé de manière non conforme, le propriétaire peut être sanctionné par une amende de 75 000 € et une peine d'emprisonnement pouvant aller jusqu'à 2 ans.`,
                },
                {
                  cas: "Véhicule ventouse",
                  descriptif: `Un véhicule ventouse est un véhicule identifiable qui stationne pendant plus de 7 jours consécutifs sans être déplacé.`,
                  qualification: `L’article L417-1 du Code de la route mentionne que « tout véhicule qui stationne sur la voie publique pendant une durée excédant la durée autorisée par la réglementation locale peut être considéré comme occupant illégalement le domaine public. Cette occupation prolongée peut entraîner une mise en demeure de déplacer le véhicule et, en cas d'inaction du propriétaire, l'enlèvement du véhicule ». Son propriétaire est passible d’une contravention de 2ème classe (pouvant aller jusqu’à 150 € d’amende). Par ailleurs, l’article 3 de la loi du 31/12/1970 complété par la loi du 15/11/2001 et le décret du 6/09/1972 définissent les conditions de mise en fourrière.`,
                },
                {
                  cas: "Véhicule gênant",
                  descriptif: `Un véhicule est gênant lorsqu’il est arrêté, stationné ou abandonné sur un emplacement non autorisé ou lorsqu’il est stationné pendant plus de 7 jours consécutifs sans être déplacé.`,
                  qualification: `Le stationnement gênant est une infraction selon l'article R417-10 du Code de la route. Il s'agit d'un stationnement qui, sans bloquer totalement le passage, rend la circulation plus difficile pour les autres usagers (piétons, cyclistes, véhicules). Cette infraction peut être sanctionnée par une amende de 2ᵉ classe, soit 35 €. Le stationnement sur des places réservées (places pour personnes handicapées, places de livraison, etc.) sans avoir droit de les occuper constitue une infraction, régie par l'article R417-11 du Code de la route, avec une amende de 135 €.`,
                },
              ],
              procedure: {
                patrimoine: {
                  titre:
                    "procedure (Véhicules identifiables sur le patrimoine)",
                  etapes: `D’abord : Identifier le propriétaire du véhicule via les voisins, ou par la police ou la municipalité. Prendre contact avec lui dans un premier temps pour déplacer le véhicule. Puis :
                  ==> Si le premier contact avec l’auteur n’a pas abouti :
                  - Envoi d’un courrier de rappel du règlement si locataire de la S.A- notamment les règles de stationnement et d’utilisation des espaces communs – et du délai de retrait du véhicule.
                  - S’il ne l’enlève pas, envoyer une mise en demeure au propriétaire avec un accusé de réception pour qu'il enlève le véhicule dans un délai de 8 jours à compter de la date de réception.
                  - Dresser un procès-verbal, si réitération de la situation.
                  - Demandez aux autorités d'organiser l'enlèvement du véhicule (mise en fourrière).
                  - Demande de remboursement à l’auteur (hors quittance) par l’agence, s’il y a eu des frais de nettoyage et d’enlèvement. Ensuite : Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.`,
                },
                proximite: {
                  titre:
                    "procedure (Véhicules identifiables à proximité du patrimoine)",
                  etapes: `D’abord : Signalez la situation aux autorités afin qu’ils vérifient si le véhicule appartient à un locataire de la S.A. HLM de l’Oise ou s’il s’agit d’un véhicule extérieur. Puis : Si le véhicule appartient à un locataire de la SA, il faut contacter le locataire pour lui rappeler les règles de stationnement et les conséquences d’un stationnement gênant.`,
                },
              },
            },
            d: {
              titre: "Travaux sauvages de mécanique",
              descriptif: `Les travaux sauvages de mécanique désignent des réparations ou modifications de véhicules effectuées sans autorisation, dans des lieux inappropriés, et souvent en violation des règles de sécurité, d’environnement ou de voisinage. Il est interdit de procéder à des travaux d’entretien, de graissage, de vidange et de réparation de véhicules sur la voie publique et notamment sur les parkings publics.
               Seuls sont autorisés les dépannages destinés à permettre au véhicule de poursuivre sa route comme un changement de roue, de batterie.`,
              qualification: `La pratique de la mécanique sauvage est passible d'une amende pouvant aller jusqu'à 135 € en cas de déversement de liquide insalubre hors des emplacements autorisés.`,
              procedure: `D’abord : Effectuer un rappel à l’ordre sur l'interdiction des travaux sauvages, et ordonner l'évacuation des équipements si ces derniers sont en cours d'utilisation. Puis :
              ==> Si l’auteur des faits est locataire de la SA HLM de l’Oise :
              - Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ;
              - Envoi d’un courrier de mise en demeure.
              - Dresser un procès-verbal.
              ==> Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.`,
            },
          },
        },
        _2: {
          titre: "Les atteintes à la tranquillité des espaces privatifs",
          contenu: {
            a: {
              titre: `Nuisances sonores, bruits et tapages injurieux ou nocturnes`,
              descriptif: `De jour ou de nuit, les nuisances sonores constituent des faits répréhensibles punis par la loi. Ils peuvent être liés au fonctionnement d’appareils ménagers, aboiements de chiens, écoute prolongée et à haut volume de musique, et autres modes d’habiter. Le caractère excessif du bruit est une notion subjective, c’est pourquoi l’anormalité du trouble est laissée à la libre appréciation du juge, qui prend en considération la durée, l’intensité et les horaires des nuisance.`,
              qualification: `« Aucun bruit particulier ne doit, par sa durée, sa répétition ou son intensité, porter atteinte à la tranquillité du voisinage ou à la santé de l’homme, dans un lieu public ou privé, qu’une personne en soit elle-même à l’origine ou que ce soit par l’intermédiaire d’une personne, d’une chose dont elle a la garde ou d’un animal placé sous sa responsabilité » - Article R1334-31 du Code la Santé Publique. Le tapage constitue une contravention de 3ème classe (art. R623-2 CP) correspondant à une amende pouvant aller jusqu’à 450 € ; néanmoins, il reste très souvent très difficile à établir.`,
              procedure: `D’abord :
              - Inviter le locataire à prendre contact avec son voisin, pour évoquer le trouble.
              - Si réitération, informer le locataire des différentes possibilités en matière de médiation : conciliateur de justice, services médiations…
              - En cas d’ouverture d’une réclamation, la gestion du trouble revient au MP pour mettre en place la procedure de TDV.
              - Si la gestion du trouble primaire n’aboutit pas, la réclamation est transférée au Pôle DSU.`,
            },
            b: {
              titre: "Nuisances liées aux animaux et chiens dangereux",
              descriptif: `Un foyer sur deux possède au moins un animal de compagnie (chiens, chats, lapins, oiseaux ou autres). Ces animaux domestiques peuvent être à l’origine de troubles dans l’habitat collectif. Certains sont susceptibles de provoquer des nuisances particulières et/ou sont soumis à des règles juridiques spécifiques. Certaines races de chiens sont soumises à un traitement plus stricte. Il existe deux catégories de chiens dits « dangereux » :
              - Les chiens d’attaque
              - Les chiens de garde et de défense Un chien qui aboie fréquemment et de manière intempestive peut être considéré comme une nuisance sonore, passible de sanctions administratives ou judiciaires. Les voisins peuvent porter plainte en cas de trouble à l'ordre public ou à la tranquillité de l’immeuble. Dans le règlement intérieur des locations, la SA HLM doit être tenue au courant de la possession d’un chien dangereux dans son patrimoine d’habitation.`,
              qualification: `La détention d’un animal « familier » est autorisée dans les immeubles collectifs, à condition qu’il ne cause « aucun dégât à l’immeuble ni aucun trouble de jouissance aux occupants de celui-ci » (aboiements intempestifs, déjections et dégradations dans les parties communes…) selon l’article 10 de la loi du 9 juillet 1970. Le propriétaire détenant un animal de compagnie perturbant le voisinage, peut recevoir une amende pour trouble sonore, qui peut être sanctionnée par des amendes de 68 à 450 € en fonction de la gravité de la nuisance. La loi n° 2008-582 du 20 juin 2008 renforce les mesures de protection des personnes contre les chiens dangereux. Elle impose de nouvelles obligations aux propriétaires et détenteurs de chiens classés dans la 1ère ou la 2e catégorie, telles que définies par les articles L.211-13-1 et L.211-14 du Code rural :
              • Obligation d'obtenir une attestation d’aptitude.
              • Obligation de soumettre tout chien âgé de plus de huit mois et de moins de 12 mois à une évaluation comportementale.
              • Obligation de détenir un permis de détention, délivré par le maire de la commune de résidence du propriétaire ou détenteur. Le fait d'exposer directement autrui à un risque immédiat de mort ou de blessures de nature à entraîner une mutilation ou une infirmité permanente par la violation manifestement délibérée d'une obligation particulière de prudence ou de sécurité imposée par la loi ou le règlement est puni d'un an d'emprisonnement et de 15 000 euros d'amende. (article 223-1 du NCP).`,
              procedure: `D’abord : Prendre l'initiative d'engager un dialogue avec le locataire concerné. Cela implique de rencontrer le locataire pour comprendre la situation, de lui rappeler les règles en matière de détention d'animaux dans le logement, et de lui expliquer l’impact des nuisances sonores sur les voisins.
              ==> Il peut être suggéré au locataire de consulter un professionnel (éducateur canin ou comportementaliste) pour résoudre les problèmes d’aboiements excessifs ou d’autres comportements nuisibles ou de trouver une solution de placement pour son animal.
              ==> Depuis les lois de 1999, 2001 et 2008, le pouvoir de police du maire en matière de chiens dangereux est renforcé, et pour cette raison le maire doit être au courant de la présence de chiens supposés de 1ère ou 2ème catégorie sur sa commune. Si c’est un chien dangereux, il faut aller demander au maire s’il est au courant de sa présence. Ensuite : Le personnel de proximité peut appeler la police municipale/police nationale/gendarmerie pour qu’ils interviennent immédiatement selon l’urgence.
              ==> Si nécessaire, il peut procéder à un constat d’huissier quant au non-respect des conditions d’hygiène et de sécurité, notamment en cas de détention d’un nombre important d’animaux dans un même logement.
              ==> Il peut saisir les autorités compétentes telles que les services vétérinaires, les SPA ou Direction Départementale de Protection des Populations de l’Oise.
              ==> Il peut enclencher une procedure de résiliation de bail. Puis :
              ==> Mettre en oeuvre des actions pour sensibiliser le locataire aux effets des bruits sur la qualité de vie des voisins, sur les sites particulièrement touchés.
              ==> Mettre en oeuvre des actions sensibilisantes au respect de toutes les obligations légales liées à la détention d’animaux dits « dangereux », sur les sites particulièrement touchés.
              ==> Si l’auteur des faits est locataire de la SA HLM de l’Oise :
              - Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ;`,
            },
          },
        },
        _3: {
          titre: "Les atteintes dû à des détournements d’usage",
          contenu: {
            a: {
              titre: "Squat de logement",
              descriptif: `Le squat désigne l’occupation d’un lieu destiné à l’habitation dans lequel une ou plusieurs personnes se sont introduites quels que soient les moyens pour le faire (effraction, menaces, contrainte). Les squatters occupent donc illégalement un logement ou local vide, voire le domicile d’un citoyen contre son gré. Il s’agit d’occupants sans droit ni titre. Ne sont pas qualifiés de squat : l’hébergement, la sous-location ou le maintien dans les lieux après la fin du bail ou une décision d’expulsion.`,
              qualification: {
                squat: {
                  titre: `Qualification juridique du Squat`,
                  continue: {
                    titre: `Occupation Continue`,
                    qualification: `Depuis la loi anti-squat du 27 juillet 2023, le squat d’un logement, qu’il soit la résidence principale ou secondaire du propriétaire, est considéré comme une violation de domicile (article 226-4 du Code pénal). Cela signifie que le propriétaire peut demander une expulsion accélérée en 48h par la préfecture (procedure administrative sans passage devant un juge). Les squatteurs risquent jusqu’à 3 ans de prison et 45 000 € d’amende. Si l'intrusion se fait par la force, la menace ou la contrainte dans le domicile d'autrui, le propriétaire ou le locataire peut demander au préfet d'ordonner l'expulsion de l'occupant en moins de 24h (article 38 de la loi du 5 mars 2007). Il n’y a pas de trêve hivernale pour les squatteurs en continue. De plus, la trêve hivernale ne s’applique pas en cas d’intrusion violente (article L.412-6).`,
                  },
                  occasionnelle: {
                    titre: `Occupation Occasionnelle`,
                    descriptif: `Depuis la loi anti-squat du 27 juillet 2023, le squat d’un logement, qu’il soit la résidence principale ou secondaire du propriétaire, est considéré comme une violation de domicile (article 226-4 du Code pénal). Cela signifie que le propriétaire peut demander une expulsion accélérée en 48h par la préfecture (procedure administrative sans passage devant un juge) . Les squatteurs risquent jusqu’à 3 ans de prison et 45 000 € d’amende. Logement inoccupé : selon l'article 315-1 du Code pénal, si les squatteurs n’occupent pas le logement de manière permanente, il peut être plus difficile de prouver un squat continu. Dans ce cas, l’expulsion devra passer par une procedure judiciaire, ce qui peut allonger les délais. Le juge pourra également ordonner une refermeture des accès afin d’éviter un éventuel retour des squatteurs. Les sanctions encourues pour une occupation illégale sont de jusqu’à 2 ans de prison et 30 000 € d’amende.`,
                  },
                },
                vol: {
                  titre: `Qualification juridique du Vol dans les parties privées`,
                  descriptif: `Un "vol dans les parties privatives" concerne les biens volés à l'intérieur d'un logement individuel (cave, box, parking…). Le vol est un délit défini par les articles 311-1 à 311-11 du Code pénal français. Le vol peut être puni par des peines variées :
                1. Vol simple (Article 311-3) : 3 ans d’emprisonnement et une amende de 45 000 €.
                2. Vol avec arme (Article 311-4) : 15 ans de réclusion criminelle et 150 000 € d’amende.
                3. Vol en bande organisée (Article 311-10) : Jusqu’à 20 ans de réclusion criminelle et 150 000 € d’amende.
                4. Vol en réunion (Article 311-10) : 10 ans de prison et 150 000 € d'amende.
                5. Vol avec des circonstances aggravantes supplémentaires / Vol avec blessures : Peines modulées en fonction de la circonstance et peines aggravées selon la gravité des blessures.
                6. Vol avec meurtre (Article 221-1) : Réclusion criminelle à perpétuité.`,
                },
              },
              procedure: `D’abord :
              => Constater la situation sans engager de confrontation directe :
              o Si des personnes sont présentes dans le logement, il est impératif de se mettre en retrait immédiatement et d’appeler le 17 (police ou gendarmerie).
              o Se faire identifier clairement en annonçant : "C'est la venue de la S.A HLM de l’Oise". Ensuite :
              • Engager la procedure d’expulsion : porter plainte et engager la procedure judiciaire d’expulsion avec la préfecture et la direction juridique.
              • Sécuriser les lieux : faire poser rapidement une fermeture sécurisée (porte, cadenas, planche en bois…) avec une entreprise.
              => S’il y a un vol pendant un squat : le propriétaire du bien volé doit déposer plainte auprès des forces de l'ordre, que l’auteur du vol soit identifié ou non. Ensuite, le sinistre doit être déclaré aux assureurs, aussi bien par le locataire que par le bailleur, selon la situation. Cette déclaration permet d’engager une demande d’indemnisation pour couvrir à la fois le vol des biens et les dommages matériels causés lors de l’effraction. Enfin :
              • Assurer une veille régulière sur l’ensemble de son patrimoine bâti et foncier (notamment sur les bâtiments concernés par des opérations de relogement).
              • Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.
              • Mettre en garde les locataires sur l’importance de restreindre l’accès au bâtiment aux personnes extérieures et de bien fermer la porte derrière eux avec l’envoi d’un courrier spécifique.`,
            },
            b: {
              titre:
                "Dépôt d’armes, de produits stupéfiants et d’objets dangereux",
              descriptif: `Les parties communes des immeubles, telles que les couloirs, les paliers, les colonnes, les caves ou les halls, peuvent parfois être détournées de leur usage normal et utilisées pour des activités illégales telles que le stockage, la consommation ou la cession de produits stupéfiants et d’objets dangereux (armes, seringues, produits chimiques, peinture, décapant, javel etc.). La présence de ces éléments constitue non seulement un risque immédiat pour la sécurité et la santé des autres habitants, mais aussi un danger environnemental, en raison des problèmes d’hygiène, des mauvaises odeurs, et des risques de toxicité.
              => Ces situations demandent donc une vigilance accrue et une réactivité rapide de la part du bailleur.`,
              qualification: `La loi impose au bailleur une obligation de sécurité. En cas de négligence dans l’enlèvement de substances dangereuses (produits chimiques, armes, ou stupéfiants), le bailleur peut voir sa responsabilité pénale engagée. L'article L129-4-1 du Code de la construction et de l’habitation précise que le bailleur doit assurer la sécurité de son bien et de ses occupants, sous peine de sanctions. En ce qui concerne spécifiquement les produits stupéfiants, l'article 222-34 du Code pénal et l'article L.3421-1 du Code de la santé publique détaillent les peines encourues pour la production, la fabrication, l’importation, l’exportation, le transport, la détention, l’offre, la cession, l’acquisition, l’emploi illicite et l’usage de stupéfiants. Les peines peuvent aller de 1 an d’emprisonnement et 3 750 € d’amende, jusqu’à la réclusion criminelle à perpétuité pour les infractions les plus graves. Le dépôt ou la mise à disposition de produits dangereux, comme des produits chimiques, seringues, ou autres objets pouvant présenter un danger pour la sécurité publique, peut entraîner des peines d'emprisonnement et des amendes. Si ces objets sont utilisés pour nuire à autrui ou causer des dommages, cela peut être puni de 2 à 5 ans de prison et d’amendes pouvant aller jusqu’à 75 000 €. L’infraction est considérée comme mise en danger d’autrui ou infraction à la sécurité publique (articles 222-19 et suivants du Code pénal). Le dépôt d'une arme dans un lieu accessible au public peut également être considéré comme une mise en danger d’autrui, une infraction punie d’une peine de prison (jusqu’à 5 ans) et d’une amende pouvant atteindre 75 000 €.`,
              procedure: `D’abord : Dès la découverte d'objets ou de produits dangereux tels que des seringues ou des produits chimiques, le bailleur doit intervenir immédiatement en veillant à respecter des conditions d’hygiène et de sécurité strictes, sans toucher les produits suscités.
              ==> Sécuriser le lieu et informer son n+1.
              ==> Appeler la police/gendarmerie, en fonction des retours. Attendre les consignes en coordination avec le n+1, pour la remise des objets. Ensuite : En cas de trafic de stupéfiants avéré, si des locataires sont impliqués, cela constitue un trouble à l’ordre public et un trouble à la jouissance paisible du logement.
              ==> Le bailleur peut alors engager une procedure de résiliation de bail, ce qui peut aboutir à l'expulsion des locataires impliqués dans ces activités illégales. Puis :
              ==> Mettre en oeuvre des actions sensibilisantes pour les locataires en cas de découverte d'armes ou d'objets dangereux (sécurité, alerte, réaction…), sur les sites particulièrement touchés : orientez les locataires vers le CAARUD (annexe adresse) qui développe des actions de médiation sociale en vue de prévenir les nuisances liées à l'usage de drogues.`,
            },
            c: {
              titre: "Occupation abusive des parties communes",
              contenu: {
                individus: {
                  tire: `Occupation des PC par les individus`,
                  descriptif: `L'occupation des parties communes fait référence à l'utilisation ou à l'appropriation temporaire ou permanente d'espaces partagés par plusieurs personnes, accompagnés de dégradations, nuisances sonores, tags, etc. Ces espaces peuvent inclure les halls d’entrée, les toits d’immeubles, les couloirs, les escaliers, les ascenseurs, les jardins ou cours partagées, les locaux techniques, à vélos ou à poubelles et les accès aux caves.`,
                  qualification: `La loi du 15/11/2001 prévoit les modalités d’intervention des services de police nationale ou unités de gendarmerie et la police municipale dans les parties communes des résidences pour rétablir la jouissance paisible des occupants lorsque ces lieux sont occupés par des personnes qui :
              - Entravent l’accès ou la libre circulation des locataires
              - Empêchent le bon fonctionnement des différents systèmes de sécurité et de sûreté
              - Nuisent à la tranquillité des lieux. L’article L126-3 du Code de la construction et de l’habitation réprime le fait d’empêcher en réunion l’accès ou la libre circulation dans les parties communes et sur les toits ou le bon fonctionnement des dispositifs de sécurité et de sûreté d’une peine de 2 mois d’emprisonnement et 3 750 € d’amende.`,
                  vol: {
                    titre: `Qualification juridique du Vol dans les PC`,
                    qualification: `Un "vol dans les parties communes" se réfère à des vols commis dans les zones accessibles à plusieurs occupants d'un immeuble. Le vol est un délit défini par les articles 311-1 à 311-11 du Code pénal français. Le vol peut être puni par des peines variées :
              6. Vol simple (Article 311-3) : 3 ans d’emprisonnement et une amende de 45 000 €.
              7. Vol avec arme (Article 311-4) : 15 ans de réclusion criminelle et 150 000 € d’amende.
              8. Vol en bande organisée (Article 311-10) : Jusqu’à 20 ans de réclusion criminelle et 150 000 € d’amende.
              9. Vol en réunion (Article 311-10) : 10 ans de prison et 150 000 € d'amende.
              10. Vol avec des circonstances aggravantes supplémentaires / Vol avec blessures : Peines modulées en fonction de la circonstance et peines aggravées selon la gravité des blessures.
              11. Vol avec meurtre (Article 221-1) : Réclusion criminelle à perpétuité. Selon l’article 311-2 du Code pénal, le vol d’électricité et eau des parties communes est assimilé à un détournement frauduleux d’énergie, passible de 3 ans d’emprisonnement et 45 000 € d’amende.`,
                  },
                  procedure: `D’abord : Constater la situation :
              • Solliciter les responsables habilités (responsables territoriaux, responsables d’agence) et prendre des photos (si vol ou dégradations).
              • Si les individus sont identifiés, un rappel du règlement intérieur doit leur être adressé, notamment en leur transmettant l’extrait relatif aux espaces communs intérieurs, qui précise que ces derniers ne peuvent être occupés de manière prolongée ni utilisés à des fins personnelles. Ensuite :
              • Faire appel à la police et/ou à la gendarmerie pour rétablir la jouissance paisible, en cas de réitération ou de nuisances. En cas de vol de matériel ou de détérioration, un signalement doit être fait immédiatement. Il convient de :
             ✔ Déposer plainte auprès des forces de l’ordre et déclarer le sinistre aux assureurs.
             ✔ Solliciter l’accès aux enregistrements de vidéosurveillance, si le site en est équipé. Puis :
              • Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.
              • Si l’auteur des faits est identifié :
              - Dresser un procès-verbal
              - Si vol : dépôt de plainte et engager une procedure de résiliation du bail en fonction de la situation.
              - Demande de remboursement à l’auteur (hors quittance ?) par l’agence. voir Anne`,
                },
                objets: {
                  titre: `Occupation des PC par des objets`,
                  descriptif: `L’occupation abusive des parties communes intègre aussi la présence d’objets permanents (meubles à chaussures, machine à laver, scooters, vélos, etc.) dans le hall, sur les paliers, dans les colonnes. Elle est strictement interdite pour garantir la sécurité, la circulation et l’usage collectif de ces espaces.`,
                  qualification: `Les parties communes doivent être dégagées pour permettre une évacuation rapide en cas d’urgence et l’accès aux services de secours. Des meubles ou objets encombrants peuvent constituer une infraction au Code de la construction et de l'habitation (article R. 111-13) s’élevant à 1500 € d’amende. Toute action ou situation entravant volontairement l'intervention des pompiers peut entraîner une amende de 7 500 € et jusqu'à 6 mois d'emprisonnement.`,
                  vol: {
                    titre: `Qualification juridique du Vol dans les PC`,
                    qualification: `Un "vol dans les parties communes" se réfère à des vols commis dans les zones accessibles à plusieurs occupants d'un immeuble. Le vol est un délit défini par les articles 311-1 à 311-11 du Code pénal français. Le vol peut être puni par des peines variées :
              6. Vol simple (Article 311-3) : 3 ans d’emprisonnement et une amende de 45 000 €.
              7. Vol avec arme (Article 311-4) : 15 ans de réclusion criminelle et 150 000 € d’amende.
              8. Vol en bande organisée (Article 311-10) : Jusqu’à 20 ans de réclusion criminelle et 150 000 € d’amende.
              9. Vol en réunion (Article 311-10) : 10 ans de prison et 150 000 € d'amende.
              10. Vol avec des circonstances aggravantes supplémentaires / Vol avec blessures : Peines modulées en fonction de la circonstance et peines aggravées selon la gravité des blessures.
              11. Vol avec meurtre (Article 221-1) : Réclusion criminelle à perpétuité. Selon l’article 311-2 du Code pénal, le vol d’électricité et eau des parties communes est assimilé à un détournement frauduleux d’énergie, passible de 3 ans d’emprisonnement et 45 000 € d’amende.`,
                  },
                  procedure: `D’abord : Constater la situation :
              • Confronter la personne pour lui expliquer la situation et qu’il procède à l’enlèvement des objets qui bloquent le passage à ses frais sous un délai.
              • Si l’objet n’a pas été enlevé sous le délai : envoi d’un courrier de rappel du règlement.
              • Envoi d’un courrier de mise en demeure.
              • Dresser un procès-verbal, en cas de réitération. Puis :
              • Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.`,
                },
                procedure: `D'abord :
* Constater la situation : Solliciter les responsables habilités (responsables territoriaux, responsables d'agence) et prendre des photos (si vol ou dégradations).
* Si les individus sont identifiés, un rappel du règlement intérieur doit leur être adressé, notamment en leur transmettant l'extrait relatif aux espaces communs intérieurs, qui précise que ces derniers ne peuvent être occupés de manière prolongée ni utilisés à des fins personnelles.
Ensuite :
* Faire appel à la police et/ou à la gendarmerie pour rétablir la jouissance paisible, en cas de réitération ou de nuisances.
* Constater la situation : Confronter la personne pour lui expliquer la situation et qu'il procède à l'enlèvement des objets qui bloquent le passage à ses frais sous un délai.
* Si l'objet n'a pas été enlevé sous le délai: envoi d'un courrier de rappel du règlement.
* Envoi d'un courrier de mise en demeure.
* Dresser un procès-verbal, en cas de réitération.
* Si la situation est récurrente, il faut :
    * Envoi d'un courrier de convocation du locataire en titre par l'agence.
    * Envoi d'un courrier de rappel du règlement au locataire en titre par l'agence.
    * Dresser un procès-verbal.
    * Enclencher une procedure de résiliation de bail.
    * Demande de remboursement à l'auteur (hors quittance) par l'agence.
Puis:
* Mettre en œuvre des actions de sensibilisation sur les sites particulièrement touchés.`,
              },
              d: {
                titre: "d. Violences urbaines ou situations à risques",
                descriptif: `La violence urbaine ou les situations à risques se réfèrent aux actes de violence, d'agression ou de vandalisme qui ont lieu dans les espaces publics des villes. Ces actes peuvent inclure des affrontements entre groupes, des attaques contre les forces de l'ordre, des incendies volontaires, des dégradations de biens publics ou privés, ainsi que des comportements menaçant la sécurité collective. Ils se manifestent fréquemment lors de rassemblements, de manifestations ou d'émeutes. `,
                qualification: `Lorsqu’un attroupement constitue une menace pour l’ordre public, les participants s’exposent à des sanctions. Pour un attroupement simple, les peines peuvent aller jusqu’à un an d’emprisonnement et 15 000 € d’amende. Si l’attroupement est armé ou violent, les sanctions sont plus sévères, pouvant atteindre trois ans d’emprisonnement et 45 000 € d’amende, conformément à l’article 431-4 du Code pénal.

              Les agressions physiques ou les actes de résistance violente envers les forces de l’ordre ou les citoyens font également l’objet de lourdes sanctions. Pour des violences légères, les peines peuvent aller jusqu’à trois ans d’emprisonnement et 45 000 € d’amende, particulièrement si elles sont commises en réunion. En cas de violences graves ayant entraîné des blessures importantes, les sanctions peuvent atteindre dix ans d’emprisonnement et 150 000 € d’amende, comme stipulé dans les articles 222-7 à 222-13 du Code pénal. `,
                procedure: `D’abord:

              Se mettre à l’abri, en sécurité, en retrait des faits. Verrouiller les locaux (loges, agences…).

              Ensuite:

              Appeler la police ou la gendarmerie pour signaler les faits.

              Puis:

              Mettre en œuvre des actions de sensibilisation sur les sites particulièrement touchés.

              Mettre en place un soutien (psychologique ou opérationnel) aux locataires/collaborateurs.

              Si l’auteur des faits est locataire de la SA HLM de l’Oise:

              Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ;

              Demande de remboursement à l’auteur (hors quittance) par l’agence`,
              },
              e: {
                titre: "e. Troubles de la santé mentale",
                descriptif: `Les troubles de la santé mentale désignent un large éventail de conditions qui affectent les pensées, les émotions, les comportements, et parfois le bien-être physique d’une personne.
              Une hospitalisation sous contrainte est imposée à une personne sans son consentement, dans un cadre légal strict.

              Une hospitalisation sans contrainte, appelée hospitalisation libre, est une prise en charge dans un établissement de santé, décidée par la personne elle-même.

              Les troubles psychiques désignent un ensemble de pathologies ou de dysfonctionnements affectant le fonctionnement mental, émotionnel ou comportemental d'une personne (troubles de l’humeur: dépression, bipolarité / troubles anxieux/ troubles liés aux addictions / troubles psychotiques: schizophrénie / troubles neurodéveloppementaux: autisme, déficit de l’attention, hyperactivité).

              Il existe deux procedures d’admission en soins psychiatriques :

              – sur décision du directeur d’établissement ;

              – sur décision du représentant de l’État (maire). `,
                qualification: `Selon l'article R 635-1 du Code pénal, toute dégradation volontaire, même si elle ne cause qu’un dommage léger, est punie d’une amende prévue pour les contraventions de 5e classe: une amende qui peut atteindre jusqu’à 1 500 €.

              L'article 322-1 al 2 du Code pénal précise que le fait de tracer des inscriptions, signes ou dessins sur des façades, des véhicules, des voies publiques ou le mobilier urbain sans autorisation préalable est un délit puni d’une amende pouvant atteindre 3 750 € et peut entraîner une peine de travail d’intérêt général. `,
                procedure: `D’abord: Evaluer la situation.

              Possibilité de demander une évaluation médicale: la famille peut contacter un médecin ou un psychiatre.

              Si la personne refuse, la famille peut demander des soins psychiatriques à la demande d’un tiers (SDT), appuyée par deux certificats médicaux.

              La famille peut solliciter le maire pour enclencher une procedure de soins sans consentement.

              Ensuite: Intervention en cas de danger imminent.

              Si le locataire met en danger sa vie ou celle des autres, la police peut intervenir immédiatement pour protéger les personnes et sécuriser les lieux.

              Intervention du SAMU qui amène à l’hospitalisation générale.

              Puis:

              Mettre en œuvre des actions de sensibilisation sur les sites particulièrement touchés: sensibiliser les voisins aux pathologies psychiques, diriger les personnes atteintes de TSM vers le personnel de santé du territoire (centre médico psychologique à Clermont), partage des numéros d’appel au niveau national.

              Déploiement de la formation de Premiers secours en Santé Mentale (PSSM) pour le personnel de proximité.`,
              },
            },
            _4: {
              titre: `Les atteintes aux biens`,
              contenu: {
                a: {
                  titre: `Graffitis sur immeuble et sur le patrimoine`,
                  descriptif: `Les graffitis ou tags sont des dessins ou inscriptions non autorisés réalisés sur des endroits non prévus à cet effet. Ces supports incluent généralement des murs, des façades de bâtiments ou du mobilier urbain. Ces actes sont considérés comme des dégradations volontaires. Selon la loi, les graffitis sont punis selon :
              - Le coût des réparations
              - L’importance du bien abîmé`,
                  qualification: `Selon l'article R 635-1 du Code pénal, toute dégradation volontaire, même si elle ne cause qu’un dommage léger, est punie d’une amende prévue pour les contraventions de 5e classe : une amende qui peut atteindre jusqu’à 1 500 €. L'article 322-1 al 2 du Code pénal précise que le fait de tracer des inscriptions, signes ou dessins sur des façades, des véhicules, des voies publiques ou le mobilier urbain sans autorisation préalable est un délit puni d’une amende pouvant atteindre 3 750 € et peut entraîner une peine de travail d’intérêt général.`,
                  procedure: `D’abord : Prendre des mesures d’urgence en cas de danger et en préservant les traces et les indices dans la mesure du possible. Ensuite : Organiser le dépôt de plainte :
              ==> Faire immédiatement remonter l’information auprès du Manager de Proximité responsable du site afin d’organiser le dépôt de plainte.
              ==> Si l’auteur des faits n’est pas identifié, déposer une plainte simplifiée systématique après chaque incident signalé.
              ==> Si l’auteur des faits est identifié, déposer une plainte dans laquelle la SA HLM précise son souhait de voir traiter ces faits par le biais de la médiation pénale. Ou enclencher la procedure d’assermentation.
              ==> Photographier les dégâts et recueillir des témoignages. Puis : Organiser le remplacement et/ou la réparation des biens et les suites à donner :
              ==> En fonction de la gravité du dommage : déclaration du sinistre auprès de l’organisme d’assurance ;
              ==> Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.
              ==> Si l’auteur des faits est locataire de la SA HLM de l’Oise :
              - Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ;
              - Envoi d’un courrier de mise en demeure ;
              - Dresser un procès-verbal ;
              - Demande de remboursement à l’auteur (hors quittance) par l’agence.`,
                },
                b: {
                  titre: `Dégradations, destruction, détérioration à caractère volontaire, vandalisme`,
                  descriptif: `Le vandalisme regroupe les faits de détérioration, de dégradation et de destruction volontaire d’un ou de plusieurs biens. Les dégradations peuvent avoir lieu : • sur le mobilier extérieur (jeux, lampadaires, etc.) ; • dans les parties communes (boites aux lettres, ascenseurs, escaliers, etc.) ; • sur les biens à usage professionnel (ordinateurs, bureaux, etc.).`,
                  qualification: `On distingue deux types de dégradations : - Les dégradations volontaires causant un dommage léger, qui constituent une contravention de 5ème classe (art. R635-1 du CP). L’amende peut aller jusqu’à 1500 €. - Les dégradations causant un dommage grave, qui sont qualifiées de délit (art. 322-1 du CP). La peine peut aller jusqu’à 2 ans d’emprisonnement et 30 000 € d’amende.`,
                  procedure: `D’abord : Prendre des mesures d’urgence en cas de danger et en préservant les traces et les indices dans la mesure du possible. Ensuite :
              ==> Faire immédiatement remonter l’information auprès du Manager de Proximité responsable du site afin d’organiser le dépôt de plainte.
              ==> Enclencher la procedure d’assermentation.
              ==> Les forces de l’ordre peuvent alors venir constater les faits, il faudra alors attendre leur constatation avant d’intervenir librement sur les lieux de la dégradation.
              ==> Photographier les dégâts et recueillir des témoignages. Puis : Organiser le remplacement et/ou la réparation des biens et les suites à donner : => En fonction de la gravité du dommage : - Déclaration du sinistre auprès de l’organisme d’assurance ; => Si l’auteur des faits est locataire de la SA HLM de l’Oise : - Envoi d’un courrier de convocation du locataire en titre par l’agence ; - Envoi d’un courrier de rappel du règlement au locataire en titre par l’agence ; - Dresser un procès-verbal ; - Demande de remboursement à l’auteur (hors quittance) par l’agence.`,
                },
                c: {
                  titre: `Incendies volontaires, involontaires, sinistres`,
                  descriptif: `Un incendie peut survenir dans diverses situations au sein de nos résidences : il peut toucher les parties communes, les logements individuels, les véhicules stationnés sur les parkings ou encore des biens appartenant au bailleur (containers, équipements divers) ou aux locataires. Selon l'origine de l'incendie, que ce soit un acte volontaire ou involontaire, la manière de réagir et les actions à entreprendre varient. La responsabilité pénale de l’auteur est engagée dans la mesure où l’incendie est la conséquence d’un manquement à une obligation de prudence ou de sécurité (ex : non-respect de l’interdiction des bonbonnes de gaz dans les logements…). Un sinistre désigne un événement soudain et imprévu qui cause des dommages matériels, immatériels ou corporels, souvent dans le cadre d'une situation assurée.`,
                  qualification: `L'incendie, selon l’article 322-5 à 322-11 du Code pénal, peut être qualifié de volontaire ou involontaire, selon les circonstances de l'incident. La loi prévoit des sanctions pénales spécifiques en fonction de l'origine de l'incendie, de la nature des biens endommagés, des risques encourus pour les personnes et de la gravité des dommages.
              • Si l'incendie est volontaire, des peines sévères sont prévues, allant d’un an d'emprisonnement et 15 000 € d'amende à des peines plus graves, pouvant aller jusqu’à la réclusion criminelle à perpétuité et une amende de 150 000 €, notamment en cas de décès ou de blessures graves liées à l'incendie.
              • Les tentatives d’incendie sont également pénalement sanctionnées.
              • En cas d'incendie involontaire, la responsabilité civile peut être engagée, en particulier si l'incendie a été causé par une négligence ou un manquement à une obligation de prudence.

              En parallèle, le locataire a l’obligation de s’assurer contre les risques d'incendie et d'autres sinistres, en vertu de l'article 7 de la loi n° 89-462 du 6 juillet 1989, modifié par la loi n° 2014-366 du 24 mars 2014 (loi pour l'accès au logement et un urbanisme rénové). Ce texte stipule que le locataire doit fournir au bailleur une attestation d'assurance lors de la remise des clés, puis chaque année à la demande du bailleur, afin de justifier de sa couverture contre les risques dont il est responsable.`,
                  procedure: `
              D’abord : Prendre des mesures d’urgence en cas de danger et en préservant les traces et les indices dans la mesure du possible. Ensuite :
              ==> Alerter les pompiers et évacuer les lieux si nécessaires.
              ==> Informer le responsable hiérarchique (N+1).
              ==> Prendre des photos des dégâts pour documenter la situation.
              ==> Dans le cadre d’une enquête, avant toute intervention technique ou de nettoyage des lieux sinistrés, le bailleur doit impérativement obtenir l’accord des services de police ou des unités de gendarmerie, ainsi que des experts d’assurance.
              ==> En cas de sinistre concernant un véhicule stationné, il revient au propriétaire du véhicule de déposer plainte auprès des autorités compétentes (police ou gendarmerie) pour que l’incident soit pris en charge. Puis :
              ==> Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.`,
                },
              },
            },
            _5: {
              titre: `À l’attention des salariés (annexe : procedure « salarié agressé »)`,
              agression: {
                verbale: {
                  titre: `Agressions verbales ou écrites`,
                  descriptif: `Injure ordinaire : Offenses graves et délibérées, paroles offensantes, affronts, outrages, insolences, crachats. Injure raciste : atteinte à une personne en fonction de sa race, de sa religion ou de ses origines. Menace : parole, geste marquant l’intention de nuire, d’affaiblir.`,
                  qualification: `L'injure non publique envers une personne, est punie de l'amende prévue pour les contraventions de la 1re classe (Article R621-2 NCP). L'injure non publique commise envers une personne ou un groupe de personnes à caractère discriminatoire (fondée sur l’origine, l’ethnie, la nation, la race, la religion, le sexe, l’orientation sexuelle ou le handicap) est punie de l'amende prévue pour les contraventions de la 4e classe. La menace de commettre des violences contre une personne, est punie de l'amende prévue pour les contraventions de la 3e classe (Article R623-1 NCP).`,
                  procedure: `Contacter votre manager pour transmettre les informations. Prendre contact avec Audrey GUIOT, référente hygiène et sécurité, pour un accompagnement. Consulter un médecin ou bien les urgences, pour obtenir un certificat médical décrivant les symptômes psychologiques. Déposer plainte en précisant la qualité de collaborateur d’un organisme HLM chargé d’une mission service public, établir une constitution conjointe de partie civile et rédiger un rapport écrit détaillé à transmettre à votre société. Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés.`,
                },
                physique: {
                  titre: `Agressions physiques avec ou sans incapacité totale de travail`,
                  descriptif: `Les agressions/violences volontaires concernent toutes les brutalités physiques avec l’intention de nuire. Le code pénal définit la gravité des violences selon le dommage causé à la victime en référence avec une interruption totale de travail personnel (ITTP). Il est indispensable de faire établir un certificat médical : Violences sans ITTP Violences avec ITTP < 8 jours Violences avec ITTP > 8 jours (constat du médecin)`,
                  qualification: `Les violences volontaires n'ayant entraîné aucune incapacité totale de travail sont punies de l'amende prévue pour les contraventions de la 4e classe (Article R624-1 NCP). Les violences volontaires ayant entraîné une incapacité totale du travail d'une durée inférieure ou égale à huit jours sont punies de l'amende prévue pour les contraventions de la 5e classe (Article R625-1 NCP).`,
                  procedure: `Faire constater les blessures par un médecin ou bien aux urgences, pour obtenir un certificat médical décrivant les blessures ou symptômes psychologiques. Déposer une plainte avec pièces justificatives : préciser sa qualité de collaborateur d’un organisme HLM chargé d’une mission service public et donner l’adresse du siège de l’organisme. Joindre un rapport détaillé à transmettre à votre société. Établir une constitution de partie civile pour renforcer la plainte. Si vous êtes victime sans ITT :
            • Envoyer un courrier d’avertissement au locataire.
            • Effectuer un dépôt de plainte en sollicitant la médiation pénale. Si vous êtes victime avec ITT :
            • Envoyer un courrier d’avertissement similaire au premier cas.
            • Privilégier un traitement particulier pour les suites judiciaires. Mettre en oeuvre des actions de sensibilisation sur les sites particulièrement touchés. Prendre contact avec Audrey GUIOT, référente hygiène et sécurité, pour un accompagnement.`,
                },
              },
            },
          },
        },
      },
      Logique_Coopération: {
        titre: `Logique de coopération entre acteurs sur la tranquillité résidentielle`,
        nuisance: {
          environnementales: {
            titre: `Nuisances environnementales et sanitaires`,
            sa_hlm: `Constats sur le terrain avec rappel du règlement intérieur si les auteurs sontidentifiés. Nettoyage des parties communes si nécessaire. Signalement des dépôts sauvages et nuisances aux services municipaux ou forces de l’ordre.`,
            mairie: `Gestion des déchets et dépôts sauvages. Application des sanctions administratives en cas de non-respect des règles.`,
            police: `Constatation des troubles à la tranquillité des locaux, identification des auteurs et mesures coercitives adaptées (contraventions, procedures pénales délictuelles).`,
          },
          sonores: {
            titre: `Nuisances sonores et comportementales`,
            sa_hlm: `terrain avec rappel du règlement intérieur si les auteurs sont identifiés et que ce rappel n’expose pas les salariés à un risque. Nettoyage et remise en état des parties communes si nécessaire. Signalement auprès des forces de l’ordre (dépôt de plainte…).`,
            mairie: `Renforcement de la surveillance préventive dans les halls concernés. Évacuations des fauteurs de troubles des lieux pour ramener la tranquillité des lieux. Verbalisation par contravention le cas échéant, voire appréhension des auteurs en cas de flagrant délit pour leur présentation devant un OPJ.`,
            police: `Constatation des troubles à la tranquillité des locaux, identification des auteurs et mesures coercitives adaptées (contraventions, procedures pénales délictuelles).`,
          },
          stationement: {
            titre: `Stationnement et véhicules`,
            sa_hlm: `Relance auprès du propriétaire lorsqu’il est connu. A défaut, une demande d'identification est adressée au commissariat.`,
            mairie: `Identification du propriétaire auprès de l’OPJ, verbalisation, immobilisation administrative et mise en fourrière`,
            police: `Identification du propriétaire, mise en demeure d’enlèvement, immobilisation administrative et mise en fourrière`,
          },
          vandalisme: {
            titre: `Dégradations et actes de vandalisme`,
            sa_hlm: `Constatation sur le terrain puis signalement aux forces de l’ordre et dépôt de plainte.`,
            mairie: `Renforcement de la surveillance préventive par le biais de sa police municipale lorsqu’il en dispose d’une. Appréhension des auteurs en cas de flagrant délit et présentation devant un OPJ.`,
            police: `Constatation de la dégradation puis ouverture d’une procedure judiciaire pour l’identification des auteurs et leur éventuelle présentation devant la justice`,
          },
          Violences: {
            titre: `Violences et sécurité publique`,
            sa_hlm: `Signalement des violences ou situations à risques aux forces de l’ordre. Assistance aux victimes si nécessaire.`,
            mairie: `Mise en place de campagnes de prévention et d’information auprès des habitants. Coordination avec les forces de l’ordre pour des actions de sécurité.`,
            police: `Intervention immédiate pour protéger les victimes et neutraliser les auteurs. Enquête et poursuites judiciaires.`,
          },
          Criminalité: {
            titre: `Criminalité et actes illégaux`,
            sa_hlm: `Signalement des comportements suspects ou infractions aux forces de l’ordre`,
            mairie: `Collaboration avec les forces de l’ordre pour organiser des actions de prévention et sécurisation des lieux.`,
            police: `Répression des infractions, saisie des objets dangereux, arrestation des auteurs, et poursuites judiciaires.`,
          },
          sante: {
            titre: `Santé publique et sociale`,
            sa_hlm: `Identification et signalement des troubles de santé mentale des aux services sociaux compétents.`,
            mairie: `Orientation des personnes en détresse vers les dispositifs municipaux d’aide sociale et dispositifs psychologiques des centres hospitaliers.`,
            police: `Intervention de la police en cas de danger immédiat pour la personne ou autrui.`,
          },
        },
      },
    },
    Axe_II: {
      titre: "Axe 2 : Outillage complémentaire à disposition",
      sections: {
        I: {
          titre: "Axe stratégique à mobiliser",
          contenu: {
            a: {
              titre: "Vidéoprotection",
              descriptif: ` -Creil : 144 Rue J-B Carpeaux (2 ext, 14 int (dont 2 de l’agence)), 84 Rue J-B Carpeaux (2 ext, 13 int), 98 Rue J-B Carpeaux (2 ext, 12 int).
              -Montataire : 38 Rue Colonel Fabien (6 int), 40 Rue Colonel Fabien (6 int), 42 Rue Colonel Fabien (6 int).
              -Beauvais : Siège (28 Rue Gambetta 60000 Beauvais – 1 caméra intérieur), Rue Ella Fitzgerald ( B1 : 2 int, 1 ext / B3 : 2 ext / B5 : 5 int, 1 ext), Rue des Vignes (A23 : 2 int / A25 : 5 int / A27 : 2 int / A29 : 2 int / A33 : 2 int / A35 : 2 int) Rue Billie Holiday (C1 : 2 int, 1 ext / C3 : 2 int / C5 : 2 int, 1 ext), Rue du Languedoc (Agence : 2 int, 2 ext / Tour A5 : 1 ext, 6 int).
              -Compiègne : 1 square Jacques Germain Soufflot (2 int, 4 ext (agence))`,
            },
            b: {
              titre:
                "Mobilisation des acteurs locaux et institutionnels clés : éléments de contacts.",
              contenu: {
                un: {
                  titre: `Hôpitaux et structures de soins`,
                  descriptif: ` -Centre Hospitalier Simone Veil de Beauvais, 40 avenue Léon Blum 60021 Beauvais 03 44 11 21 21 + Équipe de Liaison et de Soins en Addictologie (ELSA) – Centre Hospitalier Simone Veil de Beauvais, 03 44 11 21 13 elsa@ch-beauvais.fr du lundi au vendredi, de 9h à 16h30 + Service social – Centre Hospitalier Simone Veil de Beauvais, 03 44 11 20 74
              -Groupe Hospitalier Public du Sud de l'Oise – GHPSO (site de Senlis), Avenue du Docteur Paul Rougé 60309 Senlis 03 44 21 71 00
              -Centre Hospitalier Bertinot-Juel, 34 B rue Pierre Budin 60240 Chaumont-en-Vexin 03 44 49 54 54
              -Centre Hospitalier Général de Clermont, Rue Frédéric Raboisson 60600 Clermont 03 44 77 33 00
              -Centre Hospitalier Georges-Decroze, Rue Ambroise Croizat 60721 Pont-Sainte-Maxence 03 44 31 77 00
              -Centre Hospitalier Intercommunal Compiègne–Noyon, 8 avenue Henri Adnot 60200 Compiègne 03 44 23 60 00
              -Groupe Hospitalier Public du Sud de l'Oise - GHPSO (site de Creil), Boulevard Laennec 60109 Creil 03 44 61 60 00
              -Centre Médico-Psychologique (CMP) de Beauvais, 26 rue du Pont d’Arcole 60000 Beauvais Tel : 03 44 02 53 85 – Fax : 03 44 02 48 78
              -Centre Médico-Psychologique (CMP) de Clermont-de-l’Oise, 5 place Camille Sellier 60600 Clermont-de-l’Oise 03 44 78 24 87
              -Centre Médico-Psychologique (CMP) de Méru, 1 rue Henri Barbusse 60110 Méru 03 44 22 41 30
              -Clinique du Parc Saint-Lazare, 1 avenue Jean Rostand 60000 Beauvais 03 44 12 15 15
              -Hôpital de Chantilly Les Jockeys, 12 avenue du Général Leclerc 60500 Chantilly 03 44 62 66 66
              -Hôpital Local Saint-Lazare de Crépy-en-Valois, 16 rue Saint-Lazare 60800 Crépy-en-Valois 03 44 59 11 19`,
                },
                deux: {
                  titre: `Maison de Justice et du Droit`,
                  descriptif: ` -MJD de Méru et du Vexin-Thelle-Sablons : Rues Dr Graillon et Louis Bloquet, 60110 Meru. mjd-meru@justice.fr / mjd-meru@orange.fr Tel : 0344523390 Fax : 0344523399.
                -Noyon : Espace Jaurès, 11 Rue Albert de Mun, 60400 Noyon. mjd-noyon@justice.fr 0365350430.
                -Creil : 26 Rue Voltaire, 60100 Creil. MJD@creilsudoise.fr Tel : 0344644670 Fax : 0344644678.`,
                },
                trois: {
                  titre: `Conciliateurs de justice`,
                  descriptif: ` -Beauvais : Patrick Chevalier 06 62 71 47 20 patrick.chevalier@conciliateurdejustice.fr
                -Creil/Nogent-sur-Oise/Montataire : Raymond Domptail 03 44 64 46 70 (Creil) 03 44 66 30 30 (Nogent-sur-Oise) 03 44 64 44 33 (Nogent-sur-oise) raymond.domptail@conciliateurdejustice.fr
                -Auneuil : Marie-Claude Mabille 03 44 47 70 23 marie-claude.mabille@conciliateurdejustice.fr
                -Saint-Just-en-Chaussée : Françoise Salome 03 44 19 29 29 francoise.salome@conciliateurdejustice.fr
                -Méru/Noailles : Christian Villain 03 44 52 33 90 (Méru) 03 44 03 30 61 (Noailles) christian.villain@conciliateurdejustice.fr
                -Crépy en Valois/Nanteuil-le-Haudoin : Gérard Laruelle 03 44 88 38 06 (même numéro pour les 2) gerard.laruelle@conciliateurdejustice.fr
                -Neuilly-en-Thelle : Gilles Martaud 03 44 26 86 66 gilles.martaud@conciliateurdejustice.fr
                -Pont-Sainte-Maxence : André Meyer 03 44 29 48 80 andre.meyer@conciliateurdejustice.fr
                -Clermont = Xavier Gillet 03 44 50 84 84 xavier.gillet@conciliateurdejustice.fr
                -Compiègne : Françoise Angotti 03 44 38 35 24 françoise.angotti@conciliateurdejustice.fr / Alain Bonat 03 44 40 72 00 alain.bonat@gmail.com
                -Noyon : Bernard Lataix 03 44 44 28 67 bernardlataix@wanadoo.fr
                -Lassigny/Margny les Compiègne/Clairoix : Géraldine Joinct-Deboves 03 44 90 73 00 (Margny les Compiègne) 03 44 43 60 36 (Lassigny) 03 44 83 29 11 (Clairoix) geraldine.joinct-deboves@conciliateurdejustice.fr`,
                },
                quatre: {
                  titre: `SPA, refuges, associations pour animaux`,
                  descriptif: ` -SPA d’Essuilet et de l’Oise : Ferme d’Essuilet, 60510 Essuiles 03 44 48 02 50
                -SPA de Compiègne : 2 Avenue de l’Armistice, 60200 Compiègne 03 44 40 21 20
                -Fondation Clara Beauvais : 55 Chemin de la Cavée aux Pierres, 60000 Beauvais 03 44 08 42 85
                -Les Toutous & Matous de la 2è Chance : 26 Rue de l’Éventail, 60730 Sainte-Geneviève 06 66 46 38 95
                -Association Les Pa’Chats : 23 Rue du Grand Raveau, 60126 Longueil-Sainte-Marie 06 71 74 02 25
                -Association Au bonheur des Rongeurs : 60960 Feuquières 06 51 73 74 18`,
                },
              },
            },
            c: {
              titre: `procedures liées à la vie du bail et à la gestion de proximité (procedure TDV, procedure locataire récalcitrant à travaux, etc.)`,
              descriptif: `procedure salarié agressé (avec précision des 3 catégories).
            procedure TDV`,
            },
            d: {
              titre: `Soutenir les personnels de proximité par comitologie (CASSC)`,
              descriptif: `Informations`,
            },
            e: {
              titre: `L’autorisation permanente d’accéder aux parties communes des bâtiments`,
              contenu: {
                intro: `Pour la police ou les pompiers par exemple. Plus nécessaire ajd ?`,
                urgence: {
                  titre: `Pour toute situation d'urgence grave et immédiate, vous pouvez appeler :`,
                  descriptif: `15 pour le SAMU,
                18 pour les pompiers,
                17 pour la police ou la gendarmerie.
                Si vous rencontrez des difficultés à entendre ou à parler, envoyez un SMS au 114.`,
                },
                soutien: {
                  titre: `Pour un soutien spécifique, voici les numéros/sites à contacter :`,
                  descriptif: `3114 : Numéro national de prévention du suicide (24h/24, 7j/7).
                SOS Amitié : 09 72 39 40 50 (24h/24, 7j/7) ou via chat.
                Alcool Info Service : 0 980 980 930 (7j/7, 8h-2h) ou via chat.
                Drogue Info Service : 0 800 23 13 13 (7j/7, 8h-2h) ou via chat.
                Fil santé jeunes : 0 800 235 236 (7j/7, 9h-23h) ou via chat.
                Allô enfance en danger : 119 (7j/7, 24h/24) ou via chat.
                Écoute Famille Unafam : 01 42 63 03 03 (lundi au vendredi, 9h-13h et 14h-18h).
                Ces services sont anonymes et gratuits.
                Masécurité.fr`,
                },
              },
              lexique: {
                titre: `Lexique`,
                descriptif: `-Art = article
                -CAARUD = Centre d’Accueil et d’Accompagnement à la Réduction des risques pour Usagers de Drogues
                -CMP = Centre Médico-Psychologique
                -DSU = Développement Social et Urbain
                -ITTP / ITT = Incapacité Totale de Travail Personnel
                -NCP = Nouveau Code Pénal
                -OPJ = Officier de Police Judiciaire
                -PSSM = Premiers Secours en Santé Mentale
                -SPA = Société Protectrice des Animaux
                -TDV = Trouble de Voisinage
                -VHU = Véhicule Hors d’Usage`,
              },
            },
          },
        },
      },
    },
  },
};
